...
void arbolTrenes::InsertarTren(nodoTipoTren *R,int pTipoTren, int pCodTren,string pNombre, string pnumAsientos, int pCodCiudadSalida, int pCodPaisSalida){
    ...
    if(R == nullptr){
        ...
        if(R->TipoTren == pTipoTren){
            if(R->Trenes == nullptr){
                R->Trenes=new nodoTren( pCodTren, pNombre, pnumAsientos, to_string(pCodCiudadSalida), to_string(pCodPaisSalida));
                Hh = true;
                return ;
            }else{
                auxTipoTrenPadre = R;
...
void arbolTrenes::InsertarBalanceado(nodoTren *ra, int pCodTren,string pNombre, string pnumAsientos, int pCodCiudadSalida, int pCodPaisSalida){
    nodoTren *n1;
    if(ra==nullptr){
        ra=new nodoTren( pCodTren, pNombre, pnumAsientos, to_string(pCodCiudadSalida), to_string(pCodPaisSalida));
        Hh = true;
    }else{
        if(pCodTren<ra->CodTren){
            if(ra->Hizq==nullptr){
                ra->Hizq=new nodoTren( pCodTren, pNombre, pnumAsientos, to_string(pCodCiudadSalida), to_string(pCodPaisSalida));
                ra->Hizq->anterior = ra;
                Hh = true;
            }else{
        ...
        if(Hh){
            ...
            if(pCodTren>ra->CodTren){
                if(ra->Hder==nullptr){
                    ra->Hder=new nodoTren( pCodTren, pNombre, pnumAsientos, to_string(pCodCiudadSalida), to_string(pCodPaisSalida));
                    ra->Hder->anterior = ra;
                    Hh = true;
                }else{
...
bool arbolTrenes::validarCiudadSalidaRuta(int pCodTipoTren, int pCodTren, int pCodPais, int pCodCiudad){
    ...
    if(to_string(pCodPais) == pais and to_string(pCodCiudad) == ciudad){
        return true;
    }else{
        return false;
    }
...
void arbolTrenes::buscarTipoTrenParaRutaModificar(nodoTipoTren *R, int pCodTipoTren, int pCodTren, int pCodRuta, int precio, int opcion){
    if(R == nullptr){
        ...
        if(R->TipoTren == pCodTipoTren){
            buscarTrenParaRutaModificar(R->Trenes, pCodTren, pCodRuta,precio, opcion);
            return ;
        }else{
            if(R->TipoTren>pCodTipoTren){
...
void arbolTrenes::buscarTrenParaRutaModificar(nodoTren *R, int pCodTren, int pCodRuta, int precio, int opcion){
    ...
    if(R->CodTren== pCodTren){
        buscarRutaParaModificar(R->rutas, pCodRuta,precio, opcion);
        return ;
    }else{
        if(R->CodTren>pCodTren){
...
bool arbolTrenes::BuscarRuta(int pCodTipoTren, int pCodTren, string pCodPaisSalida, string pCodCiudadSalida, string pCodPaisDestino, string pCodCiudadDestino){
    RutaEncontrada = false;
    buscarRuta = "";
    BuscarTipoTrenRuta(Raiz,pCodTipoTren,pCodTren,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
    return RutaEncontrada;
}

bool arbolTrenes::BuscarRutaDijsktra(string pCodPaisSalida, string pCodCiudadSalida, string pCodPaisDestino, string pCodCiudadDestino){
    RutaEncontrada = false;
    buscarRuta = "";
    BuscarTipoTrenRutaDijsktra(Raiz,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
    return RutaEncontrada;
}



void arbolTrenes::BuscarTipoTrenRuta(nodoTipoTren *R, int pCodTipoTren, int pCodTren, string pCodPaisSalida, string pCodCiudadSalida, string pCodPaisDestino, string pCodCiudadDestino){
    if(R == nullptr){
        return ;
    }else{
        if(R->TipoTren == pCodTipoTren){
            BuscarTrenParaRuta(R->Trenes, pCodTren,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
            return ;
        }else{
            if(R->TipoTren>pCodTipoTren){
                BuscarTipoTrenRuta(R->Hizq, pCodTipoTren, pCodTren,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
            }
            if(R->TipoTren<pCodTipoTren){
                BuscarTipoTrenRuta(R->Hder, pCodTipoTren, pCodTren,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
            }
        }
    }
}

void arbolTrenes::BuscarTrenParaRuta(nodoTren *R, int pCodTren, string pCodPaisSalida, string pCodCiudadSalida, string pCodPaisDestino, string pCodCiudadDestino){
    if(R == nullptr){
        return ;
    }

    if(R->CodTren== pCodTren){
        BuscarRutaValidar(R->rutas,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
        return ;
    }else{
        if(R->CodTren>pCodTren){
            BuscarTrenParaRuta(R->Hizq, pCodTren,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
        }
        if(R->CodTren<pCodTren){
            BuscarTrenParaRuta(R->Hder, pCodTren,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
        }
    }
}

void arbolTrenes::BuscarRutaValidar(pnodoRuta R, string pCodPaisSalida, string pCodCiudadSalida, string pCodPaisDestino, string pCodCiudadDestino){
    if(R == nullptr){
        return ;
    }
    if(R->codPais == atoi(pCodPaisSalida.c_str()) and R->codCiudad == atoi(pCodCiudadSalida.c_str()) and R->codPaisDest == atoi(pCodPaisDestino.c_str()) and R->codCiudadDest == atoi(pCodPaisDestino.c_str())){
        buscarRuta = to_string(R->codPais) +" ; "+to_string(R->codCiudad) +" ; "+to_string(R->codPaisDest) +" ; "+to_string(R->codCiudadDest) +" ; "+to_string(R->precio) ;
        RutaEncontrada = true;
        return ;
    }else{
        BuscarRutaValidar(R->hi,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
        BuscarRutaValidar(R->hd,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);

    }
}

void arbolTrenes::BuscarTipoTrenRutaDijsktra(nodoTipoTren *R, string pCodPaisSalida, string pCodCiudadSalida, string pCodPaisDestino, string pCodCiudadDestino){
    if(R == nullptr){
        return ;
    }else{
        BuscarTrenParaRutaDijsktra(R->Trenes, pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
        if(!RutaEncontrada){
            BuscarTipoTrenRutaDijsktra(R->Hizq , pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
            BuscarTipoTrenRutaDijsktra(R->Hder , pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
        }
    }
}

void arbolTrenes::BuscarTrenParaRutaDijsktra(nodoTren *R, string pCodPaisSalida, string pCodCiudadSalida, string pCodPaisDestino, string pCodCiudadDestino){
    if(R == nullptr){
        return ;
    }
    BuscarRutaValidar(R->rutas,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
    if(!RutaEncontrada){
        BuscarTrenParaRutaDijsktra(R->Hizq,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
        BuscarTrenParaRutaDijsktra(R->Hder,pCodPaisSalida, pCodCiudadSalida, pCodPaisDestino, pCodCiudadDestino);
    }
