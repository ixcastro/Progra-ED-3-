...
#include "ui_window_query_basket.h"
#include "library.h"
#include "socketadmin.h"

window_query_basket::window_query_basket(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::window_query_basket)
{
    ui->setupUi(this);
...
window_query_basket::~window_query_basket()
{
    delete ui;
}

void window_query_basket::on_start_clicked()
{
    QJsonObject ob;
    ob.insert("Pasillo","");
...
void window_query_basket::on_pasilloButton_clicked()
{
    ui->halls->setEnabled(false);
    ui->pasilloButton->setEnabled(false);
...
void window_query_basket::on_prodButton_clicked()
{
    ui->prods->setEnabled(false);
    ui->prodButton->setEnabled(false);
...
void window_query_basket::on_marcaButton_clicked()
{
    ...
    QJsonObject record;
    record.insert("IsBasket",recordObjects);
    QJsonDocument docIN(record);
    QJsonDocument D = socketAdmin::getInstance()->request(docIN);
    QJsonObject ob = D.object();
    ui->listWidget->addItem(QJsonValue::fromVariant(ob.take("IsBasket")).toString());

}

void window_query_basket::on_pushButton_clicked()
{
    hide();
}
