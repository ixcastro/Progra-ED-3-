...
void MyThread::readyRead()
{
    ...
    if(json_map.firstKey() == "DJ"){
        ...
        QJsonObject x = json_map["DJ"].toJsonObject();
        QString pIdB = x.take("START").toString();
        QString pId = x.take("FINAL").toString();
        cout<<pIdB.toStdString()<<endl;
        cout<<pId.toStdString()<<endl;


        if( EST.getGrafo()->dExist(pIdB.toStdString()) && EST.getGrafo()->dExist(pId.toStdString()) ){
            EST.getGrafo()->allCero();
            EST.getGrafo()->dijkstra(pIdB.toStdString(),pId.toStdString());
            string a = EST.getGrafo()->getallData();
            QString qstr = QString::fromStdString(a);
            QJsonObject o;
            o.insert("Respuesta",qstr);
            QJsonDocument r(o);
            socket->write(r.toJson());
        }else{
            QJsonObject o;
